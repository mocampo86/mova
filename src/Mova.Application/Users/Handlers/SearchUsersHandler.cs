...
public sealed class SearchUsersHandler : ISearchUsersHandler
{
    ...
    public async Task<PagedResult<ComplexUserInfo>> HandleAsync(SearchUsersQuery query, CancellationToken cancellationToken = default)
    {
        ...
        var (items, totalItems) = await _users.SearchUsersAsync(
            query.Search,
            query.Page,
            query.PageSize,
            query.Sort,
